...
| Vue (mer/lac/montagne/dégagée…) | ❌ | `section_avis.vue_types[]` (**à ajouter**) | **Manque Lite n°1.** Pas encore collectée : `section_avis` n'a pas de clé `vue_types` (vérifié, 0 / 43 fiches en base). À ajouter en miroir de FL PR #35 (section dédiée, ~16 types atomiques + « aucune vue »). Aujourd'hui seule la case legacy `atouts_logement.vue_panoramique` existe ; elle sera retirée des atouts quand la section Vue arrivera. À ne pas confondre avec `logement_vis_a_vis` (dégagée / partielle / direct), qui est un signal d'état, pas la vue valorisable. |
